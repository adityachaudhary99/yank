...
// capabilities (TTY/color/unicode/width) and the resolved theme.
func newProgressSink(out io.Writer, f *downloadFlags, name string) progress.Sink {
	switch {
	case f.jsonOut:
	...
	return ui.NewSink(out, theme, ui.Detect(env), name)
}
