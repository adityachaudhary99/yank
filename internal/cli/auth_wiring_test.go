...
func TestNewHTTPClientLoadsCookies(t *testing.T) {
	...
	c, err := newHTTPClient(&downloadFlags{transferFlags: transferFlags{cookiesFile: jar}}, nil)
...
func TestNetrcBasicForHost(t *testing.T) {
	...
	t.Setenv("NETRC", nf)
	if got := netrcBasicFor(&downloadFlags{transferFlags: transferFlags{netrc: true}}, "https://h.example.com/f"); got != "u:p" {
		t.Fatalf("netrcBasicFor = %q", got)
	}
	if got := netrcBasicFor(&downloadFlags{transferFlags: transferFlags{netrc: true, basic: "x:y"}}, "https://h.example.com/f"); got != "" {
